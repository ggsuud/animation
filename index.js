/**
 * Created by xiaomin on 2017/3/24.
 */
'use strict';
...
function win(){
    var winAnimation = animation().loadImage(images).changePosition($rabbit3,rabbitWinMap,images[2]).repeat(3).then(function(){
        console.log("win animation repeat 3 times an finished");
...
function lose(){
    var lostAnimation = animation().loadImage(images).changePosition($rabbit4,rabbitLoseMap,images[1]).then(function(){
        console.log("lose animation finished");
        lostAnimation.dispose();
    });
    lostAnimation.start(200);
}
